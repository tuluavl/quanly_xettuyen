...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        if is_valid_password:
            ...
            # Kiểm tra xem tài khoản có thực sự BẬT 2FA trong Profile hay không
            ...
            if is_2fa_enabled:
                ...
                # 4. Kích hoạt luồng gửi mail ngầm (Thread) để không làm hoãn tiến trình chuyển trang
                threading.Thread(
                ...
                return redirect('verify_otp')  # Chuyển hướng tới view nhập OTP ngay lập tức
            # --- KẾT THÚC XỬ LÝ 2FA ---
...
def import_thi_sinh(request):
    """Đọc file Excel, xóa sạch dữ liệu cũ và import toàn bộ dữ liệu mới"""
    if request.method == 'POST' and request.FILES.get('excel_file'):
        ...
        try:
            ...
            if not df.empty:
                ...
                # Giúp khôi phục lại dữ liệu cũ nếu quá trình ghi file mới gặp lỗi
                with transaction.atomic():
                    with connection.cursor() as cursor:
                        # Xóa sạch toàn bộ dữ liệu bảng cũ (TRUNCATE nhanh hơn DELETE)
                        cursor.execute("TRUNCATE TABLE `thi_sinh_data`")
                        
                        # Chèn toàn bộ dữ liệu mới
...
def danh_sach_diem_vsat(request):
    ...
    # Thống kê số lượng thí sinh có điểm hợp lệ ở từng tổ hợp
    total_count = DiemThiVsat.objects.count()   
    stats_to_hop = {
        'total': total_count,
        'a00': DiemThiVsat.objects.filter(thmon_a00_vsat__isnull=False).exclude(thmon_a00_vsat=0).count(),
        'a01': DiemThiVsat.objects.filter(thmon_a01_vsat__isnull=False).exclude(thmon_a01_vsat=0).count(),
        'd01': DiemThiVsat.objects.filter(thmon_d01_vsat__isnull=False).exclude(thmon_d01_vsat=0).count(),
        'd07': DiemThiVsat.objects.filter(thmon_d07_vsat__isnull=False).exclude(thmon_d07_vsat=0).count(),
        'd09': DiemThiVsat.objects.filter(thmon_d09_vsat__isnull=False).exclude(thmon_d09_vsat=0).count(),
        'd14': DiemThiVsat.objects.filter(thmon_d14_vsat__isnull=False).exclude(thmon_d14_vsat=0).count(),
    }
...
def ds_in_giay_bao(request):
    ...
    context = {
        'cau_hinh': CauHinhGiayBao.get_config(),  # Thêm biến cấu hình để truyền sang HTML
        'page_obj': page_obj,
        'stats': stats,
        'query': query,
...
# CHÚ Ý: KHÔNG ĐẶT BẤT KỲ DECORATOR NÀO (như @login_required) PHÍA TRÊN HÀM NÀY
def send_otp_email_async(subject, text_content, recipient_email, html_content):
    try:
        ...
        if sent_count > 0:
            print(f"--> [THÀNH CÔNG] Đã gửi OTP đến email: {recipient_email}")
    except Exception as e:
        print(f"--> [LỖI GỬI MAIL NGẦM]: {str(e)}")